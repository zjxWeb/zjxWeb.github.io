...
[openssl相关代码](https://gitee.com/chuangdu/cppStudy/tree/master/opensll/Project1)
...
extern "C"
{
#include <openssl/applink.c>// 编译的时候将 applinc.c包含进去参与编译
};
...
string encryptPublicKey()
{
	...
	// 返回值就是密文长度
	int len = RSA_public_encrypt(text.size(), (const unsigned char*)text.data(), (unsigned char*)buf, pubKey, RSA_PKCS1_PADDING);
	// 4. 将密文返回
...
void rsaSigAndVerfiy()
{
	...
	//4. 验证签名
	int ret = RSA_verify(NID_sha1, (const unsigned char*)text.data(), 
		text.size(),(const unsigned char *)sigbuf.data(),sigbuf.size(),pubKey);
	cout << ret << endl;
}
