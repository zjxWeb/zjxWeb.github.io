...
> 无论你想用的类型是什么，Template都会生成这个函数的一个副本，只是会用实际的类型替换`T`，就好像你已经以这种方式手动输入了一样。

![72](./src/72.png)

+ 和其它的语言相似，允许你试用泛型（generic）数据类型的参数或类似的东西；

![73](./src/73.png)

## smart pointers（memory）

> **智能指针——就像一个小容器，你可以把指针进去，这个指针指向堆上的某个对象，——虽然指向堆上的对象，但他自己却存在于栈中。**
>
> **智能指针内置了一个析构函数，所以当他的作用域技术，就会释放它持有的指针**
...
+ ![76](./src/76.png)
+ 使用 `get`获取到真正的指针
+ 调用 `release()`方法，这回给你一个原始指针，但是 `unique_ptr会释放对他的所有权`

![77](./src/77.png)

> 如果 将 `unique_ptr` 作为参数传递，实际上是不起作用的；

![78](./src/78.png)

> 不能直接将一个`unique_ptr`赋值给另一个，因为这意味着他们尝试同时拥有 相同的`ListNode`； 
